...
export function setupRendering(app: Application, gameState: GameState): void {
  ...
  boardBackground.rect(0, 0, logicalBoardWidth, logicalBoardHeight).stroke({ color: 0x111111, width: 1.5, alpha: 0.9 });
  ...
  const resizeBoard = (): void => {
    ...
    const constrainedBoardWidth = Math.min(viewportWidth, viewportHeight * BOARD_ASPECT_RATIO);
